		...
		it('should successfully parse primitive types to their values', () => {
			...
			Object
				...
				.map(n => {
					...
					if (isObject) { expect(result).to.be.instanceOf((PARSABLE_VALUES as any)[n]); } // eslint-disable-line @typescript-eslint/no-explicit-any
					else { expect(result).to.be.a((PARSABLE_VALUES as any)[n]); } // eslint-disable-line @typescript-eslint/no-explicit-any
				});
